    ...
    try {
      ...
      if (userResponse.statusCode == 200) {
        ...
        setState(() {
          ...
          // current_language_id may be an int, a uuid string, or current_language object
          final langId = userData['current_language_id'] ?? userData['current_language']?['id'];
          currentLanguageId = langId?.toString();
        });
      }

      // Only call review endpoints when we have a valid language UUID
      if (currentLanguageId != null && currentLanguageId!.isNotEmpty) {
        ...
        // If no language id, skip review endpoints to avoid 403.
      }

      // (moved into conditional above)
      ...
      if (userPermissions.contains('VOCAB_WEAK_WORDS')) {
        ...
        if (weakWordsResponse.statusCode == 200) {
          ...
          // Backend denied access; ensure UI remains stable
          setState(() {
            weakWords = [];
            weakWordsCount = 0;
          });
        ...
        // No permission -> skip call and clear
        setState(() {
          weakWords = [];
          weakWordsCount = 0;
        });
      ...
      // Ensure we have a language_id: try to fall back to available languages
      ...
      if (speakingTasksResponse.statusCode == 200) {
        ...
        // No permission for speaking tasks
        setState(() {
